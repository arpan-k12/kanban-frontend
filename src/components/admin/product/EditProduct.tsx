...
import { getProductByIdAPI, updateProductAPI } from "../../../api/product.api";
...
export default function EditProduct() {
  ...
  const mutation = useMutation({
    mutationFn: (formData: FormData) => updateProductAPI(id!, formData),
    onSuccess: (data) => {
      if (data?.status) {
  ...
  const [allImages, setAllImages] = useState<(string | File)[]>([]);
  ...
  const formik = useFormik<ProductFormValues>({
    ...
    onSubmit: async (values) => {
      ...
      const keepImages = values.image.filter(
      ...
      <form onSubmit={formik.handleSubmit} className="space-y-5">
        ...
        <div>
          ...
          <input
            ...
            multiple
            accept="image/*"
            onChange={(e) => handleAddNewImages(e.currentTarget.files)}
            className="mt-1 w-full border rounded-md px-3 py-2 text-sm shadow-sm"
          />
